<div id="cc-right" style="width:20%;float:right;position:relative" class="update-nag">
	<script type="text/javascript" src="http://connect.facebook.net/en_US/all.js#xfbml=1"></script>
	<h3>Support Us</h3>
	<p>If you like this plugin, please share it with your friends</p>
	<div style="align:center;margin-bottom:15px;text-align:center">
		<a style="margin-bottom:15px" href="http://www.twitter.com/choppedcode"><img align="middle" src="http://twitter-badges.s3.amazonaws.com/follow_us-a.png" alt="Follow ChoppedCode on Twitter"/></a>
	</div>
	<div style="margin-bottom:15px;text-align:center">
		<fb:share-button href="http://www.choppedcode.com/products/mailing-list/" type="button" >
	</div>
	<p>Rate our plugin on Wordpress</p>
	<a href="http://wordpress.org/extend/plugins/mailz" alt="Rate our plugin"><img height="35px" src="http://www.choppedcode.com/wp/wp-content/uploads/sc-mcstars2.png"><img height="35px" src="http://www.choppedcode.com/wp/wp-content/uploads/sc-mcstars2.png"><img height="35px" src="http://www.choppedcode.com/wp/wp-content/uploads/sc-mcstars2.png"><img height="35px" src="http://www.choppedcode.com/wp/wp-content/uploads/sc-mcstars2.png"><img height="35px" src="http://www.choppedcode.com/wp/wp-content/uploads/sc-mcstars2.png"></img></a>
	<p>Or help us out with a small token of appreciation</p>
	<form action="https://www.paypal.com/cgi-bin/webscr" method="post">
		<input type="hidden" name="cmd" value="_s-xclick">
		<input type="hidden" name="hosted_button_id" value="SD5JE3BLJSVM6">
		<input type="image" src="https://www.paypal.com/en_GB/i/btn/btn_donate_LG.gif" border="0" name="submit" alt="PayPal - The safer, easier way to pay online.">
		<img alt="" border="0" src="https://www.paypal.com/en_GB/i/scr/pixel.gif" width="1" height="1">
	</form>
</div>
<div id="cc-right2" style="width:25%;float:right;position:relative;text-align:center;margin-top:40px">
	<a href="http://www.choppedcode.com" target="_blank"><img src="http://www.choppedcode.com/wp/wp-content/uploads/code3.png" /></a>
</div>